...
drawbar(Monitor *m)
{
	...
	if (m == selmon || 1) { /* status is only drawn on selected monitor */
                ...
                drw_setscheme(drw, scheme[SchemeNorm]);
                drw_rect(drw, x, 0, mw - stw - x, bh, 1, 1); /* to keep right padding clean */
	}
	...
	drw_map(drw, m->barwin, 0, 0, mw - stw, bh);
}
	...
	if (display != NULL) {
		...
		if (resm != NULL) {
			...
			if (xrdb != NULL) {
				...
				XRDB_LOAD_COLOR("dwm.col1", col1);
				XRDB_LOAD_COLOR("dwm.col2", col2);
				XRDB_LOAD_COLOR("dwm.col3", col3);
				XRDB_LOAD_COLOR("dwm.col4", col4);
				XRDB_LOAD_COLOR("dwm.col5", col5);
				XRDB_LOAD_COLOR("dwm.col6", col6);
...
resizebarwin(Monitor *m) {
	unsigned int w = m->ww - 2 * bargaph;
	if (showsystray && m == systraytomon(m) && !systrayonleft)
		w -= getsystraywidth();
	XMoveResizeWindow(dpy, m->barwin, m->wx + bargaph, m->by, w, bh);
}
...
updatesystray(void)
{
	...
	unsigned int sw = wstext + systrayspacing;
	unsigned int w = 1;
